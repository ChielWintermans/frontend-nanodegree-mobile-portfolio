...
var latestKnownScrollY=0;var ticking=false;function onScroll(){latestKnownScrollY=window.scrollY;requestTick();}
function requestTick(){if(!ticking){requestAnimationFrame(updatePositions);}
ticking=true;}
function updatePositions(){ticking=false;frame++;window.performance.mark("mark_start_frame");var items=document.getElementsByClassName('mover');var currentScrollY=latestKnownScrollY / 1250;var phaseArray=[];for(var i=0;i<5;i++){phaseArray.push(Math.sin(currentScrollY+(i%5)));}
var phase;for(var i=0;i<items.length;i++){phase=phaseArray[i%5];items[i].style.transform="translateX("+100*phase+"px)";}
