...
var pizzaContainers=document.querySelectorAll(".randomPizzaContainer");var pizzaContainersLength=pizzaContainers.length;var dx=determineDx(pizzaContainers[0],size);var newwidth=(pizzaContainers[0].offsetWidth+dx)+'px';function changePizzaSizes(size){for(var i=0;i<pizzaContainersLength;i++){pizzaContainers[i].style.width=newwidth;}}
...
var lastScrollY=0;var animating=false;function onScroll(){lastScrollY=window.scrollY;requestFrame();}
function requestFrame(){if(!animating){requestAnimationFrame(updatePositions);}
animating=true;}
function updatePositions(){animating=false;frame++;window.performance.mark("mark_start_frame");var currentScrollY=lastScrollY;var items=document.getElementsByClassName('mover');for(var i=0,l=items.length;i<l;i++){var phase=Math.sin((currentScrollY / 1250)+(i%5));items[i].style.transform="translateX("+100*phase+"px)";}
...
window.addEventListener('scroll',onScroll,false);document.addEventListener('DOMContentLoaded',function(){var screenWidth=screen.availWidth;var screenHeight=screen.availHeight;var s=256;var cols=screenWidth / s;var rows=screenHeight / s;var pizzaTotal=Math.ceil(cols*rows);var movingPizzas=document.getElementById("movingPizzas1");for(var i=0;i<pizzaTotal;i++){var elem=document.createElement('img');elem.className='mover';elem.src="images/pizza.png";elem.style.transform='translate3d(0, 0, 0) translate(0px)';elem.height="100";elem.width="73";elem.style.height=elem.height+'px';elem.style.width=elem.width+'px';elem.basicLeft=(i%cols)*s;elem.style.left=elem.basicLeft+'px';elem.style.top=(Math.floor(i / cols)*s)+'px';movingPizzas.appendChild(elem);}
updatePositions();});